function [K_val,alpha_bar,alpha_min] = prestab_controller_old(sys,cont)
...
for k = 1%:size(sys.H_theta_v,2)
    for j = 1:size(cont.x_v,2)
       Cons_K = [Cons_K; max(cont.H_x*(Ac(:,:,k)+sys.B0*K_var)*cont.x_v(:,j))<= cont.vec_1_x - cont.w_bar*alpha_inv; ];
    end
end
for j = 1:size(cont.x_v,2)
...
for k = 1:size(sys.H_theta_v,2)
    eigs = abs(eig(Ac(:,:,k)+sys.B0*K_val));
    if any(eigs>1)
...
for k = 1:size(sys.H_theta_v,2)
    for j = 1:size(cont.x_v,2)
        alpha_min = min(alpha_min,min(cont.w_bar./(cont.vec_1_x-cont.H_x*(Ac(:,:,k)+sys.B0*K_val)*cont.x_v(:,j))));
    end
end
